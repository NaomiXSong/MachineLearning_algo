...
a_2 = [ones(m,1) a_2'];
h_theta = sigmoid(Theta2 * a_2'); % h_theta equals z_3
...
tmp1 = Theta1(:,2:size(Theta1,2));
tmp2 = Theta2(:,2:size(Theta2,2));

% regularization formula
Reg = lambda  * (sum( sum ( tmp1.^ 2 )) + sum( sum ( tmp2.^ 2 ))) / (2*m);
...
for t=1:m,
	...
	a_2 = [1 ; a_2]; 
	z_3 = Theta2 * a_2;
	a_3 = sigmoid(z_3); % final activation layer a3 == h(theta)
	% back propag (god bless me)	

	z_2=[1; z_2]; % bias

	delta_3 = a_3 - Yk(:,t); % y(k) trick - getting columns of t element
	...
	Theta1_grad = Theta1_grad + delta_2 * a_1; % I don't know why a1 doesn't need to be transpost (brute force try)
